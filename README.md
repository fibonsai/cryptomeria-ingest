...
| `snapshot_delay` | `usize` | For Bitstamp, number of `diff_order_book` deltas to buffer before fetching the REST snapshot and merging buffered deltas by `microtimestamp` (mirrors CCXT Pro's `delta_cache_limit`). Default `6`; set `0` to fetch the snapshot immediately on connect. Ignored by OKX/Kraken/Bitvavo. See [ADR-026](docs/adr/Integration/ADR-026-20260813-bitstamp-delta-buffering-ccxt-pro.md). |
